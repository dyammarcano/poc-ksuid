...
internal class Program
{
    private static void Main()
    {
        ...
        for (var i = 0; i < total; i++)
        {
            ...
            {
                var currentId = baseId + (ulong)threadIndex;
                var ksuid = Encoder.GenerateKsuid(currentId);
...
public static class Encoder
{
    ...
    public static string GenerateKsuid(ulong id)
    {
        ...
        var idBytes = idBytesFull.Skip(2).ToArray();
    ...
    public static (ulong id, DateTime timestamp) DecodeKsuid(string ksuid)
    {
        ...
        Array.Copy(payload, 0, idBytes, 2, IdSize);
        if (BitConverter.IsLittleEndian)
